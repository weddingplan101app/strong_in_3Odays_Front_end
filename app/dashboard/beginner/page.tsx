...
import { useGetBeginnerProgramsByGenderQuery } from "@/lib/redux/api/programsApi"

export default function BeginnerPage() {
  const { data: menData, isLoading: loadingMen, error: menError } = useGetBeginnerProgramsByGenderQuery("male")
  const { data: womenData, isLoading: loadingWomen, error: womenError } = useGetBeginnerProgramsByGenderQuery("female")
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-6">
      <div>
        <h1 className="text-3xl font-bold text-foreground mb-2">{menData?.title || "Beginner Programs"}</h1>
        <p className="text-muted-foreground">{menData?.description || "30-day programs for those starting their fitness journey"}</p>
      </div>
