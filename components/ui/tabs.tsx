...
function TabsList({
  ...
  return (
    <TabsPrimitive.List
      data-slot="tabs-list"
      className={cn(
        // keep existing muted background but ensure subtle contrast in dark mode
        'bg-muted text-muted-foreground inline-flex h-9 w-fit items-center justify-center rounded-lg p-[3px] dark:bg-muted/30',
        className,
      )}
      {...props}
    />
...
function TabsTrigger({
  ...
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        // Make the active tab visually distinct in both light and dark modes
        "data-[state=active]:bg-primary/10 data-[state=active]:text-primary data-[state=active]:border-primary data-[state=active]:shadow-sm",
        "dark:data-[state=active]:bg-primary/20 dark:data-[state=active]:text-primary",
        // keep focus and base styles
